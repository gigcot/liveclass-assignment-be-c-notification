...
class UserNotificationTest {
    ...
    class StatusTransition {
        ...
        @Test
        @DisplayName("이미 QUEUED 상태에서 markQueued 재호출 시 멱등 처리")
        void shouldBeIdempotentWhenMarkQueuedFromQueued() {
            UserNotification notification = createNotification();
            notification.markQueued();

            assertThatCode(notification::markQueued).doesNotThrowAnyException();
        }

        @Test
        @DisplayName("SENT 상태에서 QUEUED 전이 시 예외")
        void shouldThrowWhenMarkQueuedFromSent() {
            UserNotification notification = createNotification();
            notification.markQueued();
            notification.claim();
            notification.markSent();
